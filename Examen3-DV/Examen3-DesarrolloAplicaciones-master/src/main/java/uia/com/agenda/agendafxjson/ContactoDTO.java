...
public class ContactoDTO extends Contacto{
    ...
    private ObservableList<EventoDTO> itemsEventoDTO = FXCollections.observableArrayList();
    ...
    public ContactoDTO(InfoAgenda infoAgenda)
    {
        ...
        if(infoAgenda.getItems().size()>0) {
            for(int i=0; i<infoAgenda.getItems().size(); i++)
            {
                ...
                this.getItemsEventoDTO().add(new EventoDTO(infoAgenda.getItems().get(i)));
            }
        }
    }
    ...
    public ObservableList<EventoDTO> getItemsEventoDTO() {
